...
class _FakeLangBase:
    runtime_option_specs: dict[str, object] = MappingProxyType({})
    runtime_option_aliases: dict[str, str] = MappingProxyType({})
    setting_specs: dict[str, object] = MappingProxyType({})
...
class TestCmdDetect:
    ...
    def test_unknown_detector_exits(self, monkeypatch):
        ...
        class FakeLang(_FakeLangBase):
            name = "typescript"
            detect_commands = MappingProxyType(
                {"unused": lambda a: None, "smells": lambda a: None}
            )
            large_threshold = 300
    ...
    def test_valid_detector_dispatches(self, monkeypatch):
        ...
        class FakeLang(_FakeLangBase):
            name = "typescript"
            detect_commands = MappingProxyType(
                {"unused": lambda a: calls.append("unused")}
            )
            large_threshold = 300
    ...
    def test_large_threshold_default(self, monkeypatch):
        ...
        class FakeLang(_FakeLangBase):
            name = "typescript"
            detect_commands = MappingProxyType(
                {"large": lambda a: captured_args.append(a)}
            )
            large_threshold = 500
    ...
    def test_dupes_threshold_default(self, monkeypatch):
        ...
        class FakeLang(_FakeLangBase):
            name = "typescript"
            detect_commands = MappingProxyType(
                {"dupes": lambda a: captured_args.append(a)}
            )
            large_threshold = 300
    ...
    def test_hyphen_alias_resolves_to_underscore_key(self, monkeypatch):
        ...
        class FakeLang(_FakeLangBase):
            name = "python"
            detect_commands = MappingProxyType(
                {"single_use": lambda a: calls.append("single_use")}
            )
            large_threshold = 300
    ...
    def test_passthrough_alias_maps_to_props_when_passthrough_missing(
        ...
        class FakeLang(_FakeLangBase):
            name = "python"
            detect_commands = MappingProxyType({"props": lambda a: None})
            large_threshold = 300
    ...
    def test_runtime_options_are_passed_without_lang_run_shim(self, monkeypatch):
        ...
        class FakeLang(_FakeLangBase):
            ...
            large_threshold = 300
            runtime_option_specs = MappingProxyType({"roslyn_cmd": object()})
            runtime_option_aliases = MappingProxyType({"roslyn_cmd": "roslyn_cmd"})
