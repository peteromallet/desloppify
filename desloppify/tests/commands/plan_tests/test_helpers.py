...
def test_state_path_uses_runtime_project_root_without_constant_patch(
    monkeypatch,
    tmp_path,
):
    """state_path uses runtime root resolution when PROJECT_ROOT constant is untouched."""
    state_dir = tmp_path / ".desloppify"
    state_dir.mkdir(parents=True, exist_ok=True)
    expected = state_dir / "state-python.json"
    expected.write_text("{}", encoding="utf-8")

    from desloppify.app.commands.helpers import state as state_helpers

    monkeypatch.setattr(state_helpers, "get_project_root", lambda: tmp_path)
    args = SimpleNamespace(state=None, lang="python", command="next")
