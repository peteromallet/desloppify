...
from desloppify.core.skill_docs import SkillInstall  # noqa: E402
...
class TestCmdUpdateSkill:
    ...
    def test_valid_interface(self, mock_update):
    ...
    @patch("desloppify.app.commands.update_skill.find_all_installed_skills", return_value=[])
    def test_no_interface_found(self, _mock_find, _mock_colorize, capsys):
    ...
    @patch("desloppify.app.commands.update_skill.colorize", side_effect=lambda t, _c: t)
    def test_unknown_interface(self, _mock_colorize, capsys):
    ...
    @patch("desloppify.app.commands.update_skill.update_installed_skill")
    @patch("desloppify.app.commands.update_skill.find_all_installed_skills")
    def test_updates_all_installed(self, mock_find, mock_update):
        mock_find.return_value = [
            SkillInstall(rel_path=".claude/skills/desloppify/SKILL.md", version=1, overlay="CLAUDE", stale=True),
            SkillInstall(rel_path=".cursor/rules/desloppify.md", version=1, overlay="CURSOR", stale=True),
        ]
        args = argparse.Namespace(interface=None)
        cmd_update_skill(args)
        assert mock_update.call_count == 2
        mock_update.assert_any_call("claude")
        mock_update.assert_any_call("cursor")


class TestUpdateInstalledSkill:
    @patch("desloppify.app.commands.update_skill.colorize", side_effect=lambda t, _c: t)
    def test_unknown_interface(self, _mock_colorize, capsys):
        result = update_installed_skill("unknown-interface")
    ...
    @patch("desloppify.app.commands.update_skill.colorize", side_effect=lambda t, _c: t)
    @patch("desloppify.app.commands.update_skill._download")
    def test_download_decode_failure(self, mock_download, _mock_colorize, capsys):
        mock_download.side_effect = OSError("downloaded SKILL.md is not valid UTF-8")
        result = update_installed_skill("claude")
        assert result is False
        out = capsys.readouterr().out
        assert "Download failed" in out
    ...
    @patch("desloppify.app.commands.update_skill.colorize", side_effect=lambda t, _c: t)
    @patch("desloppify.app.commands.update_skill._download")
    @patch("desloppify.app.commands.update_skill.safe_write_text")
    def test_write_failure_returns_false(
        self,
        mock_safe_write_text,
        mock_download,
        _mock_colorize,
        capsys,
        tmp_path,
        ...
        mock_safe_write_text.side_effect = OSError("disk full")
